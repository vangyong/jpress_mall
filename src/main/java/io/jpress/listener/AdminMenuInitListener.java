...
public class AdminMenuInitListener implements MessageListener {
	...
	public void onMessage(Message message) {
		...
		//menuMnager.addMenuGroup(createRoleMenuGroup());
		menuMnager.addMenuGroup(createMallMenuGroup());
	...
	private MenuGroup createTransactionMenuGroup() {
	...
	private MenuGroup createAlipayMenuGroup() {
		MenuGroup group = new MenuGroup("alipay", "fa fa-qrcode", "支付宝");
		{
			group.addMenuItem(new MenuItem("option", "/admin/option/alipay", "支付宝设置"));
		}
		return group;
	}
	...
	private MenuGroup createAttachmentMenuGroup() {
		...
		{
			group.addMenuItem(new MenuItem("list", "/admin/attachment/list?status=&m="+ Consts.MODULE_MALL, "所有附件"));
			group.addMenuItem(new MenuItem("index", "/admin/attachment?m="+ Consts.MODULE_MALL, "附件一览"));
			group.addMenuItem(new MenuItem("upload", "/admin/attachment/upload?m="+ Consts.MODULE_MALL, "上传"));
		}
		return group;
	}
	...
	private MenuGroup createUserMenuGroup() {
		...
		{
			group.addMenuItem(new MenuItem("list", "/admin/user", "后台用户"));
			group.addMenuItem(new MenuItem("edit", "/admin/user/edit", "添加"));
			group.addMenuItem(new MenuItem("info", "/admin/user/info", "我的资料"));
			group.addMenuItem(new MenuItem("member", "/admin/user/member", "会员用户"));
		}
		return group;
	}
	
//	private MenuGroup createRoleMenuGroup() {
//		MenuGroup group = new MenuGroup("role", "fa fa-user", "角色");
//		{
//			group.addMenuItem(new MenuItem("list", "/admin/role", "所有角色"));
//			group.addMenuItem(new MenuItem("edit", "/admin/role/edit", "添加"));
//		}
//		return group;
//	}

	private MenuGroup createMallMenuGroup() {
		MenuGroup group = new MenuGroup("mallOption", "fa fa-shopping-cart", "商城配置");
		{
			group.addMenuItem(new MenuItem("banner", "/admin/template/banner", "首页banner图"));
			group.addMenuItem(new MenuItem("menu", "/admin/template/menu", "导航菜单"));
			group.addMenuItem(new MenuItem("spec", "/admin/spec", "商品规格"));
		}
		return group;
	}

	private MenuGroup createTemplateMenuGroup() {
		MenuGroup group = new MenuGroup("template", "fa fa-magic", "页面模板");
		{
...
//		MenuGroup group = new MenuGroup("addon", "fa fa-plug", "系统插件");
//		{
