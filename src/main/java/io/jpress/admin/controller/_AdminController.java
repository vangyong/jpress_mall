...
public class _AdminController extends JBaseController {
	...
	public void login() {
		...
		User user = UserQuery.me().findUserByUsernameAndFlag(username, User.FLAG_ADMIN);
		...
		if (StringUtils.isBlank(user.getStatus()) || User.STATUS_FROZEN.equals(user.getStatus())) {
			renderAjaxResultForError("该用户已被冻结！");
			return;
		}

		if(user.isVisitor()){
			renderAjaxResultForError("该用户没有登录权限！");
			return;
		}
