...
public class _AttachmentController extends JBaseCRUDController<Attachment> {
	...
	public void index() {
		...
		Page<Attachment> page = AttachmentQuery.me().paginate(getPageNumber(), getPageSize(), null, null, null, null,
				getPara("k", "").trim(), getDm(), getMime(), null, Attachment.STATUS_NORMAL, null);
	...
	public void list() {
		keepPara();
		Page<Attachment> page = AttachmentQuery.me().paginate(getPageNumber(), getPageSize(), null, null, null, null, getPara("k", "").trim(), getDm(), getMime(), null, getStatus(), null);
		setAttr("page", page);
		setAttr("status", getStatus());
	...
	public void changeStatus(){
		Attachment attachment = AttachmentQuery.me().findById(getParaToBigInteger("id"));
		if(attachment != null){
			attachment.setStatus(getPara("status"));
			attachment.saveOrUpdate();
	...
	public void doUpload() {
		...
		if (null != uploadFile) {
			...
			//附件审核
			String status = Attachment.STATUS_NORMAL;
			Boolean attachment_must_audited = OptionQuery.me().findValueAsBool("attachment_must_audited");
			if (attachment_must_audited != null && attachment_must_audited) {
				status = Attachment.STATUS_DRAFT;
			}

			Attachment attachment = new Attachment();
			attachment.setModule(getM());
