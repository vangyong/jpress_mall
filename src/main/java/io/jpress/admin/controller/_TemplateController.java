...
public class _TemplateController extends JBaseController {
	...
	public void banner() {
		...
		setAttr("metadata", metadata);

		List<Metadata> metadataList = MetaDataQuery.me().findListByTypeAndId(Consts.INDEX_BANNER, Consts.INDEX_BANNER_ID);
	...
	protected boolean validate(Map<String, String> metas){
		for(int x = 0 ; x < metas.size() ; x++) {
			if (metas.get("index_banner" + x + "_img") == null) break;
			if (metas.get("index_banner" + x + "_img").isEmpty()) {
				renderAjaxResultForError("banner图不能为空！");
				return false;
			}
			if (metas.get("index_banner" + x + "_remark") == null) break;
			if (metas.get("index_banner" + x + "_remark").isEmpty()) {
	...
	public void doClearBanners() {
		BigInteger id=getParaToBigInteger("id");
		if(id!=null){
			MetaDataQuery.me().deleteByTypeAndId(Consts.INDEX_BANNER, id);
	...
	public void doSaveBanners() {
		...
		boolean saved = Db.tx(new IAtom() {
			...
			public boolean run() throws SQLException {
				//删除
				MetaDataQuery.me().deleteByTypeAndId(Consts.INDEX_BANNER, Consts.INDEX_BANNER_ID);
				...
				if (!metas.isEmpty()) {
					for (Map.Entry<String, String> entry : metas.entrySet()) {
						...
						metadata.setObjectId(Consts.INDEX_BANNER_ID);
						metadata.setObjectType(Consts.INDEX_BANNER);
						metadata.save();
					}
				}
