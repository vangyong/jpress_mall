...
public class Content extends BaseContent<Content> implements ISortModel<Content> {
	...
	public void setText(String text) {
		if(StringUtils.isNotBlank(text)){
			...
			text=text.replaceAll("(<img[^>]*?)\\s+width\\s*=\\s*\\S+","$1");
			text=text.replaceAll("(<img[^>]*?)\\s+height\\s*=\\s*\\S+","$1");
			text=text.replaceAll("<(img class=\"image fit\")(.*?)","<img");
			text=text.replaceAll("<(img)(.*?)","<img class=\"image fit\"");
		}
		super.setText(text);
	}
