...
public class MetaDataQuery extends JBaseQuery {
	...
	public Metadata findByTypeAndIdAndKey(String type, BigInteger id, String key) {
		...
		return DAO.doFindFirst("object_type = ? and object_id = ? and meta_key = ? ", type, id, key);
	}
	
	public int deleteByTypeAndId(String type, BigInteger id) {
		return DAO.doDelete("object_type = ? and object_id = ?", type, id);
	}
