...
public class ContentQuery extends JBaseQuery {
	...
	public Content find(BigInteger id, BigInteger spec_value_id) {
		StringBuilder sql = new StringBuilder();
		sql.append(" SELECT c.id,c.title,c.thumbnail,CONCAT(s.title,'：',sv.value) AS value,csi.price,csi.spec_value_id ");
		sql.append(" FROM content c ");
		sql.append(" LEFT JOIN contentspecitem csi ON c.id=csi.content_id ");
		sql.append(" LEFT JOIN specvalue sv ON sv.id=csi.spec_value_id ");
		sql.append(" LEFT JOIN spec s ON sv.spec_id=s.id ");
		sql.append(" WHERE 1=1 ");
		sql.append(" AND csi.content_id = "+id);
